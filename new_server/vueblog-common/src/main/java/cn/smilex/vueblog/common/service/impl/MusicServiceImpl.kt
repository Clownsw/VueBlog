...
import cn.smilex.vueblog.common.util.CollectionUtils
import cn.smilex.vueblog.common.util.CommonUtils.Companion.CollectionToStrMap
import cn.smilex.vueblog.common.util.CommonUtils.Companion.calcLimit
import cn.smilex.vueblog.common.util.CommonUtils.Companion.calcPageCount
import cn.smilex.vueblog.common.util.CommonUtils.Companion.collectionToStr
...
import cn.smilex.vueblog.common.util.CommonUtils.Companion.tryRunNoReturn
...
@Slf4j
@Service
open class MusicServiceImpl(private val vueBlogConfig: VueBlogConfig) : ServiceImpl<MusicDao, Music>(), MusicService {
    companion object {
        @JvmStatic
    ...
    private fun checkServer() {
        ...
        if (StringUtils.isBlank(vueBlogConfig.musicServer)) {
            throw VueBlogException(ResultCode.ERROR_SYSTEM_CONFIG_ERROR)
        }
    }
        ...
        return tryRun(
            {
                val httpResponse = RequestUtils.get(
                    String.format(
                        ...
                        vueBlogConfig.musicServer,
                        collectionToStr(
                            idList, CollectionToStrMap { obj: Any -> obj.toString() },
                            ","
                        )
                    )
                )
